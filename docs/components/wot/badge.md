...
| 属性名称 | 类型 | 默认值 | 是否必填 | 说明 |
|---------|------|--------|---------|------|
| modelValue | Number / String / null | null | 否 | 徽章显示值 |
| max | Number | 99 | 否 | 最大值，超过显示 max+ |
| isDot | Boolean | false | 否 | 是否点状徽章 |
| hidden | Boolean | false | 否 | 是否隐藏徽章 |
| type | String | 'danger' | 否 | 类型，可选值：primary / success / warning / danger / info |
| bgColor | String | - | 否 | 自定义背景色 |
| color | String | - | 否 | 自定义字体色 |
| top | Number | - | 否 | 上下偏移量 |
| right | Number | - | 否 | 左右偏移量 |
