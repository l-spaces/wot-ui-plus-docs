...
|---------|---------|---------|-------------|
| update:value | 选项切换时触发 | (index: number) | 当前选中的索引 |
...
通过 `options` 传入选项数组，`mode` 设置模式，使用 `v-model:value` 绑定当前索引。

```vue
<template>
  <wd-segmented :options="list" mode="box" v-model:value="current" @change="onChange" />
  <wd-segmented :options="list" mode="button" v-model:value="current" @change="onChange" />
</template>
...
```vue
<template>
  <wd-segmented :options="list" mode="box" shape="round" v-model:value="current" />
  <wd-segmented :options="list" mode="button" shape="round" bar-color="#3c9cff" active-color="#fff" v-model:value="current" />
  <wd-segmented :options="list" mode="box" v-model:value="current" active-color="#f56c6c" />
</template>
...
```vue
<template>
  <wd-segmented :options="list" mode="button" v-model:value="current" />

  <wd-segmented :options="customList" v-model:value="current2" height="80px" @change="onChange">
...
- 组件当前使用 `value` / `update:value` 作为双向绑定入口，示例应使用 `v-model:value`
