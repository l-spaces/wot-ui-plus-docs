...
| 属性名称 | 类型 | 默认值 | 是否必填 | 说明 |
|---------|------|--------|---------|------|
| modelValue | Number | 0 | 否 | 选中的值（时间戳），支持 v-model |
...
默认按周切换模式，通过 `v-model` 绑定选中日期。

```vue
<template>
  <wd-date-strip v-model="value" @change="onChange" />
</template>
...
```vue
<template>
  <wd-date-strip v-model="value1" mode="none" :min-date="minDate" :max-date="maxDate" />
  <wd-date-strip v-model="value2" bg-color="#f5f5f5" active-bg-color="#ff6b35" active-color="#fff" round="10px" />
  <wd-date-strip v-model="value3" active-mode="date" height="80px" item-width="45px" item-round="100px" />
</template>
...
```vue
<template>
  <wd-date-strip v-model="value" :show-lunar="true" height="70px" />
  <wd-date-strip v-model="value2" :disabled-date="disabledDates" :disabled-fun="disabledFun" mode="none" />
</template>
